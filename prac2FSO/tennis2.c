...
void* mou_paleta_usuari(void * arg)
{
  ...
  { 
    ...
    if (((tecla) == TEC_AVALL) && (win_quincar(ipu_pf+l_pal,ipu_pc) == ' '))
    {
      ...
      printf("Moviments 1: %i",moviments);
    }
    if (((tecla) == TEC_AMUNT) && (win_quincar(ipu_pf-1,ipu_pc) == ' '))
    {
      ...
      if (moviments > 0) moviments--;    /* he fet un moviment de la paleta */
      printf("Moviments 2: %i",moviments);
    }    

  } while (tecla != TEC_RETURN && (moviments > 0 || moviments == -1));
...
{
  //pthread_mutex_lock(&mutex_paleta_o);
  int f_h;
 /* char rh,rv,rd; */
  ...
  do
  {
    ...
    //pthread_mutex_unlock(&mutex_paleta_o);
...
int main(int n_args, const char *ll_args[])
{
  ...
  //Aqui va el thread_join en lloc del bucle do-while
  int t;
  pthread_join(paleta_u, (void **)&t);
 ...
 */
   pthread_mutex_destroy(&pilota_mutex);
   win_fi();
